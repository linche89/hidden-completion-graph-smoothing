...
\thanks{Author names, affiliations, funding, and the corresponding-author
contact must be inserted before submission. TSIPN uses single-anonymized
peer review.}}
...
\begin{IEEEkeywords}
Graph signal processing, graph Laplacian regularization, hidden nodes,
distributed estimation, robust optimization, semidefinite programming,
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/certificate_sampling_failure.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.58\linewidth]{figures/scenario_scaling.pdf}
...
A separate factorial audit covers $h\in\{0,1,2,4,8\}$, three edge-weight
ranges ($[0.01,0.1]$, $[0.2,5]$, and $[10,100]$), three requested mean
degrees, and three repetitions per cell, for 135 physical completions in
total.  For $h>0$, the realized visible--hidden cuts range from 2 to 15 edges
and from $0.0344$ to $508.44$ in total weight.  Every sampled operator error
lies below its independently computed exact finite-$h$ certificate; the
largest error-to-certificate ratio is $0.9475$.  This is a coverage and
implementation audit only: finite physical samples remain lower bounds and do
not replace the exact supremum.

Table~\ref{tab:p4-ablation} collects the certificate failures and the safe
interpretation of each deliberately weakened baseline.

\begin{table}[t]
  \centering
  \small
  \input{tables/p4_ablation.tex}
  \caption{Certificate and NO-GO ablations on the principal theorem-matched
  instance.  Positive gaps quantify what is lost by sampling, deleting
  partition directions, or replacing the exact terminal geometry by a coarse
  outer set.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/local_method_tradeoff.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/external_stress.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.58\linewidth]{figures/cpu_gpu_crossover.pdf}
